...
int main( int argc, char** argv )
{
	...
	cin >> srcFileName;
	Mat srcImg = imread(srcFileName, IMREAD_GRAYSCALE);
	if(srcImg.empty())
	{
	...
	/// 自动阈值分割（Otsu阈值分割）
	Mat bwImg;
	threshold(srcImg, bwImg, 0, 255, THRESH_BINARY | THRESH_OTSU);
	...
	Mat distImg, foreImg;
	distanceTransform(openImg, distImg, DIST_L2, 5);
	double maxDist;	// 最大距离
	minMaxLoc(distImg, 0, &maxDist);
	threshold(distImg, foreImg, 0.68*maxDist, 255, THRESH_BINARY);
	...
	findContours(foreImg, contours, hierarchy, RETR_CCOMP, CHAIN_APPROX_SIMPLE);
	...
	{
		drawContours(markerImg, contours, idx, Scalar::all(compCount+1), FILLED, 8, hierarchy);
	}
