...
int main( int argc, const char** argv )
{
	...
	/// 阈值分割
	Mat bwImg;
	cv::threshold(srcImg, bwImg, 50, 255, THRESH_BINARY_INV | THRESH_OTSU);
	...
	findContours( bwImg, contours, hierarchy, RETR_CCOMP, CHAIN_APPROX_SIMPLE );
	...
	if( !contours.empty() && !hierarchy.empty() )
	{
		...
		for( ; idx >= 0; idx = hierarchy[idx][0] )
		{
			...
			drawContours(dstImg, contours, idx, color, FILLED, 8, hierarchy);
		}
	}
