...
void onKernelSizeTrackbar(int iVal, void* pUserData)
{
	...
	case 'H':	// Homogeneous Blur（均值滤波）
		...
		putText(dstImg, "Homogeneous Blur", Point(dstImg.cols/4, dstImg.rows/2),FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(255,255,255));
		break;
	case 'B':	// Bilateral Blur（双边滤波）
		bilateralFilter(*pSrcImg, dstImg, kernelSize, kernelSize*2, kernelSize/2); 
		putText(dstImg, "Bilateral Blur", Point(dstImg.cols/4, dstImg.rows/2),FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(255,255,255));
		break; 
	case 'G':	// Gaussian Blur（高斯平滑）
		GaussianBlur(*pSrcImg, dstImg, Size(kernelSize, kernelSize), 0, 0);
		putText(dstImg, "Gaussian Blur", Point(dstImg.cols/4, dstImg.rows/2),FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(255,255,255));
		break;
	case 'M':	// Median Blur（中值滤波）
		medianBlur(*pSrcImg, dstImg, kernelSize);
		putText(dstImg, "Median Blur", Point(dstImg.cols/4, dstImg.rows/2),FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(255,255,255));
		break;
	}
